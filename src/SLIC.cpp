	...
	const double&				M)
{
	...
	for( int itr = 0; itr < 10; itr++ )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
                        ...
                        y1 = std::max((double)0,kseedsy[n]-offset);
                        y2 = std::min((double)m_height,	kseedsy[n]+offset);
                        x1 = std::max((double)0,			kseedsx[n]-offset);
	...
	const double&				m)
{
	...
	for( int itr = 0; itr < 4; itr++ )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
                        y1 = max((double)0,			kseedsy[n]-offset);
                        y2 = min((double)m_height,	kseedsy[n]+offset);
                        x1 = max((double)0,			kseedsx[n]-offset);
                        x2 = min((double)m_width,	kseedsx[n]+offset);
                        z1 = max((double)0,			kseedsz[n]-offset);
		...
		for( int d = 0; d < m_depth; d++  )
		{
			...
			for( int r = 0; r < m_height; r++ )
			{
				for( int c = 0; c < m_width; c++ )
				{
					sigmal[klabels[d][ind]] += m_lvec[ind];
					sigmaa[klabels[d][ind]] += m_avec[ind];
					sigmab[klabels[d][ind]] += m_bvec[ind];
